...
##Education factors
  ...
  esz<-array(0,dim=c(dim(mb_samp_sp)[1],9))
  pv_sp<-array(0,dim=c(dim(mb_samp_sp)[1],9)) #p value 
  kl<-5
...
for(kl in c(203:207,360:363,365:369,228:229)){
...
colnames(effect_size_phen)[(lim+1):dim(effect_size_phen)[2]]<-colnames(phen_all_use)[c(203:207,360:363,365:369,228:229)]
...
write.csv(effect_size_phen,'effect_size_phen_6_lmer.csv')
write.csv(pval_phen,'pval_phen_6_lmer.csv')
write.csv(fdr_phen,'fdr_phen_6_lmer.csv')
